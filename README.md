...
This circuit will be design using three D flip flops. In each of the D flip flop the clock component will connect to a clock and then the resulting output will connect to the and gates.
![Schematic](Flip-Flop-Schematic.svg)
...
Data type structures such as reg and wire were used to get the various states and outputs as for reg it would represent the flip flops much better and also is able to register bits and for this simulation the states are represented as three bits. Also, the outputs are also bits as well which is why in the Verilog code outputs are in data type reg as well. All of this was implemented using continuous and behavioral blocks and assignments. Due to the fact that there are states it would be appropriate in the Verilog to use procedural assignments such as always.
## Analysis
